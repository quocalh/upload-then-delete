...
SkyBox_CubemapTexture = CubeMapTextureCombineTexture(r"User\Objects\skybox.png")

WaterVertices = CreateVertices(2000, 2000, 0.5, (0.4, 0.7, 0.8))
...
while running:
    ...
    WaterShader.Bind()
    UserCamera.SetUniformForCamMatrix(WaterShader, "Perspective")
    WaterShader.SetUniformVec3("CameraPosition", UserCamera.Position)
    WaterShader.SetUniformFloat("fTime", Wavetime)
    WaterShader.SetUniformVec3("SunPosition", Sun_Position)
    ...
    Triangle.VAO.Bind()
    WaterVertices_IBO.Bind()
    glDrawElements(GL_TRIANGLES, len(WaterVertices.Indices), GL_UNSIGNED_INT, None)
    WaterShader.Unbind()
